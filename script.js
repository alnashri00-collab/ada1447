...
// جلب البيانات من Web App مع معالجة الأخطاء
fetch(webAppUrl)
  .then(res => res.json())
  .then(data => {
    console.log("Data fetched:", data);
    if (!data || Object.keys(data).length === 0) {
      container.innerHTML = "<p style='color:red; text-align:center;'>لا توجد بيانات من Web App. تحقق من النشر وأذونات الوصول.</p>";
      return;
    }
    ...
    Object.keys(data).forEach(key => {
      ...
      elem.files.forEach(file => {
        ...
        label.innerHTML = `
          <input type="checkbox" class="fileCheckbox" data-url="${file.url}" data-ext="${ext}"> 
          ${file.name} <img class="icon" src="${icon}" />
        `;
  ...
  .catch(err => {
    ...
    container.innerHTML = "<p style='color:red; text-align:center;'>خطأ في جلب البيانات من Web App. تحقق من الرابط والأذونات.</p>";
  });
...
document.getElementById('printBtn').addEventListener('click', ()=>{
  ...
  </head><body><div id="printContainer">`;
  ...
  selected.forEach(input => {
    ...
    if(['jpg','jpeg','png'].includes(ext)){
      ...
      html += `<a href="${url}" target="_blank">فتح الملف ${input.parentElement.textContent.trim()}</a>`;
    }
    html += `</div>`;
  });
